# 🛡️ Clean Architecture Checker Skill for Antigravity
...
**`clean-arch-checker`**는 Google Antigravity 에이전트에서 iOS(Swift), Android(Kotlin), Flutter(Dart) 프로젝트의 **클린 아키텍처(Clean Architecture)** 및 **의존성 역전 원칙(DIP)** 준수 여부를 자동으로 점검하고, 리팩토링 방안을 제시하는 스킬 패키지입니다.
...
* **진단 & 자동 수정 지원**: 단순 점검 리포트 제공부터 사용자의 요청 시 에이전트를 통한 **코드 자동 리팩토링**까지 지원

---

### 💻 빠른 설치 (NPX 사용)

터미널에서 아래 단 한 줄의 명령어만 실행하면 사용자의 Antigravity 환경에 스킬이 자동 설치됩니다.

#### 1. 전역(Global) 스킬로 설치 (추천)
모든 프로젝트에서 공통으로 사용하도록 `~/.gemini/config/skills/`에 설치합니다:
...
현재 작업 중인 레포지토리의 `.agents/skills/`에만 설치합니다:
...
설치가 완료되면 Antigravity 에이전트에게 다음과 같이 자연어로 요청하세요:
...
**`clean-arch-checker`** is an Antigravity agent skill package designed to automatically audit iOS (Swift), Android (Kotlin), and Flutter (Dart) projects for compliance with **Clean Architecture** principles and the **Dependency Inversion Principle (DIP)**.
...
Run a single command in your terminal to automatically install the skill into your Antigravity environment.

#### 1. Global Installation (Recommended)
Installs into `~/.gemini/config/skills/` for use across all projects on your machine:
...
Installs into `.agents/skills/` of your current workspace repository:
...
Once installed, simply prompt your Antigravity agent in plain language:
